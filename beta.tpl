...
Author: Gergely Daróczi (@daroczig)
Data required: FALSE
Packages: nortest
...
Example: rapport(fp = "foo", data = "fd77ad1f23409841873e08defe1f6ef4",     n = 100, shape1 = 0.1, shape2 = 0.1)
head-->
...
add links to wiki + nortest pklg

The skewness is <%=skewness(g)%> and the kurtosis is <%=kurtosis(g)%> based on the <%=n%> observations, although most frequentists would be rather interested in the mean (<%=mean(g)%>) and standard deviation (<%=sd(g)%>.

But did we generated a normal distribution after all?
